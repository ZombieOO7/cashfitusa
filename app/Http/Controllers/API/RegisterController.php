...
class RegisterController extends BaseController
{
    ...
    public function __construct(User $user,EmailTemplate $emailTemplate)
    {
        ...
        $this->fillableCols = ['id','first_name', 'last_name', 'email', 'phone', 'position_id', 'company_id'];
    }
    ...
    public function register(Request $request)
    {
        ...
        $rules = [ 'email' => 'required|email|unique:users,email,NULL,id,deleted_at,NULL', 'first_name' => 'required|max:50', 'last_name' => 'required|max:50', 'phone' => 'required', 'device_id' => 'required', 'fcm_token' => 'required', 'device_type' => 'required', 'company_id' => 'required|exists:companies,id','position_id' => 'required'];
        /** Validation code end */
            ...
            if (count($userDataArr) == 0) {
                ...
                try  {
                    ...
                    if (isset($createdUser->uid)) {
                        ...
                        return $this->getResponse($userArr,true,200,__('api_messages.register.account_created'), $userObj->createToken(config('constant.oAuthAccessTokensName'))->accessToken);  /** Display data to frontend code. */
                    } else {
    ...
    public function setRequestValue($request)
    {
        ...
        array_set($request, 'user_type', $request->position_id); 
        return $request;
    }
    ...
    public function getUserArrResponse($request, $userObj)
    {
        ...
        $userArr['position']['position_id'] = (int) $request->position_id;
