...
{
    private $helper, $user;
    public $viewConstant = 'admin.wallet-transaction.';
    public function __construct(WalletTransactionHelper $helper, User $user)
    {
        $this->helper = $helper;
        $this->user = $user;
    ...
    public function create($uuid = null)
    {
        try {
            ...
            $title = isset($uuid) ? trans('formname.wallet-transaction.update') : trans('formname.wallet-transaction.create');
...
public function store(WithdrawTransactionFormRequest $request, $uuid = null)
    {
        ...
        try {
            ...
            if ($request->has('id') && !empty($request->id)) {
                $msg = __('admin/messages.action_msg', ['action' => __('admin/messages.updated'), 'type' => 'Wallet Transaction']);
            } else {
                $msg = __('admin/messages.action_msg', ['action' => __('admin/messages.created'), 'type' => 'Wallet Transaction']);
            }
            return redirect()->route('wallet-transaction.index')->with('message', $msg);
        } catch (Exception $e) {
    ...
    public function destroy(Request $request)
    {
        ...
        try{
            if (isset($request->id)) {
                ...
                $this->helper->dbEnd();
                $msg = __('admin/messages.action_msg', ['action' => __('admin/messages.deleted'), 'type' => 'Wallet Transaction']);
    ...
    public function multidelete(Request $request)
    {
        ...
        try{
            ...
            if ($request->action == config('constant.inactive') || $request->action == config('constant.active')) {
                ...
                $msg = __('admin/messages.action_msg', ['action' => $action, 'type' => 'Wallet Transaction']);
                return response()->json(['msg' => $msg, 'icon' => 'success']);
            } else {
                $msg = __('admin/messages.action_msg', ['action' => __('admin/messages.deleted'), 'type' => 'Wallet Transaction']);
                return response()->json(['msg' => @$msg, 'icon' => 'success']);
