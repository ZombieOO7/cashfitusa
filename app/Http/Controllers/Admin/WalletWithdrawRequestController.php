...
use App\Helpers\WalletWithdrawHelper;
...
class WalletWithdrawRequestController extends BaseController
{
    private $helper, $user;
    public $viewConstant = 'admin.wallet-withdraw.';
    public function __construct(WalletWithdrawHelper $helper, User $user)
    {
        $this->helper = $helper;
        $this->user = $user;
...
public function store(WithdrawTransactionFormRequest $request, $uuid = null)
    {
        ...
        try {
            ...
            if ($request->has('id') && !empty($request->id)) {
                $msg = __('admin/messages.action_msg', ['action' => __('admin/messages.updated'), 'type' => 'Wallet Withdraw Request']);
            } else {
                $msg = __('admin/messages.action_msg', ['action' => __('admin/messages.created'), 'type' => 'Wallet Withdraw Request']);
            }
    ...
    public function destroy(Request $request)
    {
        ...
        try{
            if (isset($request->id)) {
                ...
                $this->helper->dbEnd();
                $msg = __('admin/messages.action_msg', ['action' => __('admin/messages.deleted'), 'type' => 'Wallet Withdraw Request']);
    ...
    public function multidelete(Request $request)
    {
        ...
        try{
            ...
            if ($request->action == config('constant.inactive') || $request->action == config('constant.active')) {
                ...
                $msg = __('admin/messages.action_msg', ['action' => $action, 'type' => 'Wallet Withdraw Request']);
                return response()->json(['msg' => $msg, 'icon' => 'success']);
            } else {
                $msg = __('admin/messages.action_msg', ['action' => __('admin/messages.deleted'), 'type' => 'Wallet Withdraw Request']);
                return response()->json(['msg' => @$msg, 'icon' => 'success']);
