...
(function ($)
{
    ...
    //Generate data table
    c._generateDataTable = function(element_id_name,ajax_URL,field_coloumns,order_coloumns,extraParam=null,data,dom){
        var bSearching = true;
        ...
        table = $('#'+element_id_name).DataTable({
            ...
            initComplete: function () {
                ...
                if(element_id_name == 'loan_transaction_table'){
    ...
    if (input.files && input.files[0]) {
        var reader = new FileReader();
         if(input.files[0].type == 'image/jpeg' || input.files[0].type == 'image/jpg' || input.files[0].type == 'image/png'){
            reader.onload = function (e) {
                $(document).find('.image-block').show();
...
function readsURL(input) {
    if (input.files && input.files[0]) {
        var reader = new FileReader();
            ...
            reader.onload = function (e) {
                $(document).find('.image-block').show();
                $('#image_upload_previews').attr('src', e.target.result);
            }
            reader.readAsDataURL(input.files[0]);
        }
    }
