...
@section('title', __('formname.faq-category.list'))
        ...
        <div class="m-portlet m-portlet--mobile">
            <div class="m-portlet__body">
                <div class="m-form__content">
                    <h5>{{__('formname.faq-category.list')}}</h5>
                </div>
                <hr>
                    ...
                    <div class="m-portlet__head-caption">
                        <div class="m-portlet__head-title">
                            ...
                            <a href="javascript:;" class="btn btn-primary submit_btn" id='action_submit'
                                data-url="{{route('faq-category.multi_delete')}}"
                                data-table_name="faq_category_table" data-job_url="{{ route('faq-category.multipleJobStatus') }}">{{__('formname.submit')}}</a>
                            <button class="btn btn-info" style='margin:0px 0px 0px 12px' id='clr_filter'
                                data-table_name="faq_category_table" >{{__('formname.clear_filter')}}</button>
                        </div>
                    </div>
                ...
                <!--begin: Datatable -->
                <table class="table table-striped- table-bordered table-hover table-checkable for_wdth " id="faq_category_table"
                    data-type="" data-table_name="faq_category_table" data-url="{{ route('faq-category.datatable') }}" data-job_url="{{ route('faq-category.multipleJobStatus') }}">
                    <thead>
                        <tr>
                            <th class="nosort">
                            ...
                            <th>{{__('formname.faq-category.name')}}</th>
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <th><input type="text" class="form-control form-control-sm tbl-filter-column"
                                    placeholder="{{__('formname.faq-category.name')}}"></th>
                                    <th><input type="text" class="form-control form-control-sm tbl-filter-column"
...
@section('inc_script')
<script>
var url = "{{ route('faq-category.datatable') }}";
</script>
<script src="{{ asset('backend/js/faq-category/index.js') }}" type="text/javascript"></script>
@stop